...
export default class PaginaPrincipal{
   ...
   private readonly botaoFecharModalPassageiro: Locator;
   ...
   private readonly botaoComprar: Locator;
    ...
    constructor (page: Page){
        ...
        this.botaoFecharModalPassageiro = page.getByTestId('fechar-modal-passageiros');
    ...
    async clicarSomenteIda(){
            await this.botaoSomenteIda.click();
        }

    async clicarSelecionarPassageiros(){
            await this.botaoAbrirModalPassageiros.click();
        }

    async adicionarQuantidadeAdultos(quantidade: number){
            for ( let i = 1; i < quantidade; i ++ )  {
                await this.botaoIncrementarAdultos.click();
            }
        }
    async adicionarQuantidadeCriancas(quantidade: number){
            for ( let i = 0; i < quantidade; i ++ )  {
                await this.botaoIncrementarCriancas.click();
            }
        }
     async adicionarQuantidadeBebes(quantidade: number){
            for ( let i = 0; i < quantidade; i ++ )  {
    ...
    async fecharModalPassageiros(){
            await this.botaoFecharModalPassageiro.click();
    }
  ...
  async definirOrigemEDestino(origem: string, destino: string) {
    ...
    async clicaBotaoBuscarPassagens(){
